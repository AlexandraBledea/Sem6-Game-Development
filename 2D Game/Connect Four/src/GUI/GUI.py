...
class GUI:
    ...
    def __init__(self, board, service):
        ...
        self._scorePlayer1 = 0
        self._scorePlayer2 = 0
        self._board = board
    ...
    def start_of_the_game(self):
        ...
        self._screen.fill(self._black)
        label = self._print_font.render("Type 1 to start", True,
                                        self._second_piece)
        self._screen.blit(label, (0, 0))
        label = self._print_font.render("Scores:", True, self._second_piece)
        self._screen.blit(label, (0, 70))
        label = self._print_font.render("Player 1: " + str(self._scorePlayer1), True, self._second_piece)
        self._screen.blit(label, (0, 110))
        label = self._print_font.render("Player 2: " + str(self._scorePlayer2), True, self._second_piece)
        self._screen.blit(label, (0, 150))
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_RETURN:
                        ...
                        self.validate_start_input(user_text.strip())
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_RETURN:
                        ...
                        self.validate_second_input(user_text.strip())
                        self.finalize_choosing_the_side(user_text.strip())
    ...
    def simulate_circle_falling(self, row, selected_column, piece):
        # pygame.draw.circle(self._screen, self._first_piece,
        #                    (int(selected_column * self._square_size + self._square_size / 2), self._height -
        #                     int(row * self._square_size + self._square_size / 2)), self._radius)
        starting_y = int(self._square_size / 2)
        speed = 5
        print(self._height - int(row * self._square_size + self._square_size / 2))
        while starting_y <= self._height - int(row * self._square_size + self._square_size / 2):
            # now = pygame.time.get_ticks()
            # if now - self._last >= self._cooldown:
            pygame.draw.rect(self._screen, self._black, (0, 0, self._width, self._square_size))
            pygame.draw.circle(self._screen, piece, (int(selected_column * self._square_size + self._square_size / 2),
                                                     starting_y + speed), self._radius)
            starting_y += speed
            # self._last = now
            pygame.display.update()
            pygame.time.wait(20)
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if turn == 0:
                        ...
                        if self._board.is_free(selected_column):
                            ...
                            if self.simulate_circle_falling(row, selected_column, self._first_piece):
                        ...
                        if self._board.is_free(selected_column):
                            row = self._board.get_new_open_row(selected_column)
                            if self.simulate_circle_falling(row, selected_column, self._second_piece):
                                self._board.place_piece(row, selected_column, 2)

                                if self._service.winning_move(2):
                                    self._scorePlayer2 += 1
